    ...
    while (true) {
        ...
        try {
            ...
            if (detection.level === 'hard') {
                ...
                await (0, worker_broadcast_1.broadcastLog)('Soft anti-bot signal after error. Cooling down briefly.', 'warn');
...
function getClosestByReach(posts, settings) {
...
async function detectCaptcha() {
    ...
    try {
        ...
        if (level !== 'none') {
            ...
            await (0, worker_broadcast_1.broadcastLog)(`CAPTCHA / anti-bot signal (${level}): ${reason} at ${info.url}`, level === 'hard' ? 'error' : 'warn').catch(() => { });
        }
        return {
            level,
            reason,
