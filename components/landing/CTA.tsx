'use client';

import React from 'react';
import { motion } from 'motion/react';
import Link from 'next/link';

export default function CTA() {
  return (
    <section className="py-24 px-4 bg-black">
      <div className="max-w-[980px] mx-auto">
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="text-center"
        >
          <h2 className="text-display-hero text-white mb-4">
            Ready to grow?
          </h2>
          <p className="text-xl text-[rgba(255,255,255,0.56)] mb-10 max-w-xl mx-auto" style={{ fontWeight: 400, lineHeight: 1.19 }}>
            Join thousands of professionals automating their LinkedIn presence. Start your 14-day free trial today.
          </p>

          {/* Apple Pill CTAs */}
          <div className="flex items-center justify-center gap-4 mb-12">
            <Link href="/login?mode=register">
              <button className="btn-apple-primary text-base px-6 py-3 rounded-[980px]">
                Start Free Trial
              </button>
            </Link>
            <button className="btn-apple-pill-dark px-6 py-3">
              Schedule Demo
            </button>
          </div>

          {/* Trust */}
          <div className="flex flex-wrap justify-center gap-8 text-caption text-[rgba(255,255,255,0.48)]">
            <span>No credit card required</span>
            <span className="text-[rgba(255,255,255,0.16)]">·</span>
            <span>14-day free trial</span>
            <span className="text-[rgba(255,255,255,0.16)]">·</span>
            <span>Cancel anytime</span>
          </div>
        </motion.div>
      </div>
    </section>
  );
}
