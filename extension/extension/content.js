...

{
  ...
  async function extractPipeline(keyword, settings, comments, dashboardUrl, userId, runId) {
    ...
    const syncConstraints = {
      ...
      // Search-Only: LinkedIn's own search engine already filtered by keyword — re-applying the
      // keyword gate in JS silently drops posts whose text snippets didn't render in time,
      // or were harvested before their DOM fully loaded. Always skip it for Search-Only.
  ...
  async function syncPosts(posts, keyword, dashboardUrl, userId, linkedInProfileId, isFinal = false, constraints = {}) {
    ...
    if (isFinal && SEARCH_ONLY_FINAL_RANKING) {
      ...
      let qualified = eligibleCandidates
        .filter(p => p.reachScore >= adaptiveFloor)
        .map(p => ({ ...p, qualificationReason: `score>=${adaptiveFloor}` }));
      // IMPORTANT: Drop the adaptive floor only when qualified === 0, exactly mirroring
      // the scroll loop's getSearchOnlyCommitSnapshot() logic. Using `< TARGET_MIN` here
      // was wrong — it would change the floor when 1-9 posts qualified, producing a
      // different count in syncPosts than what the scroll logs showed as "qualified".
